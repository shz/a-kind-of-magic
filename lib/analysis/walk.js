...
//
// Note that we do need to skip FunctionDeclarations since they
// won't be entered.
//
nodeHandlers['FunctionDeclaration'] = {enter: function() { return SKIP }};
...
nodeHandlers['MemberExpression'] = {
  enter: function(node, parent, state) {
    var result = memberWalker(node);
    var output = result.output;

    // Add what scope we can
    if (state.scope['function'][0][output[0]] || !state.scope.has(output[0])) {
      for (var i=0; i<output.length; i++) {
        if (output[i].match(/\.(prototype|hasOwnProperty)$/))
          break;
        state.requires.push(output[i]);
      }
    }

    // Process any relevant expressions in the MemberExpression chain
    result.expressions.forEach(function(exp) {
      estraverse.traverse(exp, state.traverser);
    });
    return SKIP;
  }
};
  ...
  enter: function(node, parent, state) {
    ...
    if (node.left.type == 'MemberExpression') {
      ...
      if (result.fullyChained) {
        var rootName = result.output[0];
        if (state.scope.has(rootName)) {
          if (state.scope.has('builtin', rootName) || state.scope['function'][0].hasOwnProperty(rootName))
            state.scope['function'][0][result.output.pop()] = true;
          else
            state.scope.add('function', result.output.pop()); // FIXME - This is a hack
        } else {
          state.scope['function'][0][result.output.pop()] = true;
        }
      }
...
//
// ObjectExpression keys are expressed as identifiers and will be
// added as requires unless we manually avoid them, which is what
// this bit does.
//
nodeHandlers['ObjectExpression'] = {
  enter: function(node, parent, state) {
    node.properties.forEach(function(p) {
      if (p.kind == 'init')
        estraverse.traverse(p.value, state.traverser);
    });
    return SKIP;
  }
};
...
var walk = module.exports = function(ast, requires, provides) {
  ...
  for (var i in state.scope['function'][0]) if (state.scope['function'][0].hasOwnProperty(i))
    provides.push(i);
  // Push global state as well
  state.scope.list('global').forEach(function(s) { provides.push(s) });
};
